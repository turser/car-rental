...
function Section({ title, children }) {
    return (
        <div className="bg-white border border-stone-200 rounded-lg shadow-sm mb-4 overflow-hidden">
            {title && (
...
function DataTable({ headers, rows, empty }) {
    if (empty) return <p className="text-sm text-stone-400">{empty}</p>;
    return (
        <table className="w-full text-sm">
            <thead>
                <tr className="border-b border-stone-100">
                    {headers.map(h => <th key={h} className="pb-2.5 text-left text-xs font-semibold text-stone-500 uppercase tracking-wider pr-4">{h}</th>)}
                </tr>
            </thead>
            <tbody className="divide-y divide-stone-50">{rows}</tbody>
        </table>
    );
}
...
export default function VoitureDetail() {
    ...
    if (loading) return (
        <div className="space-y-4 max-w-4xl">
            ...
            {[...Array(3)].map((_, i) => <div key={i} className="h-28 bg-stone-200 rounded-lg animate-pulse" />)}
        </div>
    );
        ...
        <div className="max-w-4xl">
            ...
            <div className="flex items-center justify-between gap-3 mb-6">
                <div className="flex items-center gap-3">
                    <button onClick={() => navigate('/voitures')} className="text-sm text-stone-500 hover:text-stone-900 transition flex items-center gap-1.5 font-medium">
                        <i className="ti ti-arrow-left text-[14px]" /> Retour
                    </button>
                    <div className="w-px h-4 bg-stone-300" />
                    <h1 className="text-xl font-semibold text-stone-900">{car.brand} {car.model}</h1>
            ...
            <Section>
                <div className="flex gap-6">
                    <div className="w-48 h-36 rounded-lg overflow-hidden flex-shrink-0 bg-stone-100 border border-stone-200 flex items-center justify-center">
                        {img
                            ? <img src={IMAGE_BASE + img.image_path} alt={car.brand} loading="lazy" className="w-full h-full object-cover" />
            ...
            <Section title="Assurances">
                <DataTable
                    ...
                    rows={car.insurances?.map(ins => (
                        <tr key={ins.id}>
                            <td className="py-2.5 pr-4 font-medium text-stone-800">{ins.company}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtPrice(ins.price)}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtDate(ins.start_date)}</td>
                            <td className="py-2.5 text-stone-600">{fmtDate(ins.end_date)}</td>
                        </tr>
                    ))}
                />
            </Section>
            ...
            <Section title="Taxes">
                <DataTable
                    ...
                    rows={car.taxes?.map(tax => (
                        <tr key={tax.id}>
                            <td className="py-2.5 pr-4 font-medium text-stone-800">{tax.year}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtPrice(tax.amount)}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtDate(tax.due_date)}</td>
                            <td className="py-2.5">
            ...
            <Section title="Maintenances">
                <DataTable
                    ...
                    rows={car.maintenances?.map(m => (
                        <tr key={m.id}>
                            <td className="py-2.5 pr-4 font-medium text-stone-800">{m.type}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtPrice(m.cost)}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{fmtDate(m.date)}</td>
                            <td className="py-2.5 pr-4 text-stone-600">{m.mileage?.toLocaleString()} km</td>
                            <td className="py-2.5 text-stone-600">{m.next_due_mileage?.toLocaleString()} km</td>
                        </tr>
                    ))}
                />
            </Section>
            ...
            <Section title="Historique des locations">
                <DataTable
                    ...
                    rows={car.rentals?.map(r => {
                        ...
                        return (
                            <tr key={r.id}>
                                <td className="py-2.5 pr-4 text-stone-600">{fmtDate(r.start_date)}</td>
                                <td className="py-2.5 pr-4 text-stone-600">{fmtDate(r.end_date)}</td>
                                <td className="py-2.5 pr-4 text-stone-600">{fmtPrice(r.price_per_day)}</td>
                                <td className="py-2.5 pr-4 font-semibold text-stone-900">{fmtPrice(r.total_price)}</td>
                                <td className="py-2.5 pr-4 text-stone-600">{fmtPrice(r.paid_amount)}</td>
                                <td className="py-2.5">
