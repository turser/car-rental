...
export default function ClientDetail() {
    ...
    if (loading) return (
        <div className="space-y-4 max-w-3xl">
            ...
            <div className="h-28 bg-stone-200 rounded-lg animate-pulse" />
        </div>
    );
    ...
    return (
        <div className="max-w-3xl">
            ...
            <div className="flex items-center gap-3 mb-6">
                <button onClick={() => navigate('/clients')} className="text-sm text-stone-500 hover:text-stone-900 transition flex items-center gap-1.5 font-medium">
            ...
            <Section title="Historique des locations">
                {client.rentals?.length === 0 ? (
                    <p className="text-sm text-stone-400">Aucune location enregistrée.</p>
                ) : (
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-stone-100">
                                {['Début', 'Fin', 'Prix/jour', 'Total', 'Payé', 'Statut'].map(h => (
                                    <th key={h} className="pb-2.5 text-left text-xs font-semibold text-stone-500 uppercase tracking-wider pr-4">{h}</th>
                                ))}
                            </tr>
                        </thead>
